...
print STDERR "TARGETDIR=" . $root . "\n";
print STDERR "TOOLCHAIN=" . $uclibc. "\n";
print STDERR "SRCBASE=" . $ENV{"SRCBASE"} . "\n";
...
#	fixDynDep("libbcm.so", "libc.so.0");
	...
	fixDynDep("sqlite3", "libsqlite3.so.0.8.6");
...
#shibby
genSO("${root}/usr/lib/libcurl.so.4.4.0", "${router}/libcurl/lib/.libs/libcurl.a", "", "-L${router}/zlib -L${router}/openssl");
